...
if (!$reference) {
    header("Location: ../frontend/garage.php?payment_error=" . urlencode("No payment reference found."));
    exit;
}
...
if (!$result || !$result['status']) {
    header("Location: ../frontend/garage.php?payment_error=" . urlencode("Payment verification failed. Please contact support."));
    exit;
}
...
if ($status === 'success') {
    ...
    header("Location: ../frontend/payment_success.php?reference=" . urlencode($reference));
    exit;
} else {
    ...
    header("Location: ../frontend/garage.php?payment_error=" . urlencode("Payment was not successful. Status: " . $status));
    exit;
}
?>
